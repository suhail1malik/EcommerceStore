  ...
  return (
    <div className="space-y-6">
      {orders.map((order) => {
        ...
        return (
          <div key={order._id} className="premium-card rounded-[24px] p-6 flex flex-col sm:flex-row gap-6 hover:shadow-2xl hover:shadow-emerald-500/10 transition-all duration-500 group relative overflow-hidden">
            <div className="absolute inset-0 bg-gradient-to-r from-emerald-500/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity" />
            
            <div className="flex-shrink-0 relative z-10">
              <div className="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700/50 bg-slate-50 dark:bg-slate-900 shadow-sm">
                <img
            ...
            <div className="flex flex-col justify-between sm:items-end sm:ml-4 border-t sm:border-t-0 sm:border-l border-slate-100 dark:border-slate-800 pt-5 sm:pt-0 sm:pl-6 mt-4 sm:mt-0 relative z-10">
              ...
              <Link
                to={`/order/${order._id}`}
                className="flex items-center justify-center gap-2 px-5 py-2.5 bg-slate-900 dark:bg-slate-700 text-white hover:bg-black dark:hover:bg-slate-600 rounded-xl text-[10px] font-bold uppercase tracking-widest transition-all active:scale-95 shadow-sm"
              >
                Inspect Manifest &rarr;
              </Link>
            </div>
          </div>
        );
